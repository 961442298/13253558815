...
public interface EmployeeDao extends JpaRepository<Employee, Integer>,JpaSpecificationExecutor<Employee> {
	...
	@Query(value = "SELECT * FROM employee WHERE employee_card=?1", nativeQuery = true)
	Employee findByEmployeeCard(String employeeCard); 
	@Query(value = "SELECT roleid FROM employeewith_role WHERE employeeid=?1", nativeQuery = true)
	List<Integer> queryRoleIdByEid(Integer employeeId);
}
